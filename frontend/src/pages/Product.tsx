...
interface Order {
  _id: string;
  items: { productId: string | { _id: string }; quantity: number; size: string }[];
  status: string;
}

const Product: React.FC = () => {
  const { productId } = useParams<{ productId: string }>();
  const { currency, addCartItem, backendUrl, token, csrfToken, fetchCsrfToken } = useContext(shopContext)!;
  ...
  const [hasReviewed, setHasReviewed] = useState(false); // New state for checking if user has reviewed
...
if (!currentCsrfToken) {
  ...
  currentCsrfToken = fetchedToken;
      ...
      if (reviewResponse.data.success) {
        ...
        const userHasReviewed = reviews.some(review => review.userId._id === userId);
...
if (!currentCsrfToken) {
  ...
  currentCsrfToken = fetchedToken;
      ...
      if (response.data.success) {
        ...
        await Promise.all([fetchReviews(), fetchProductData(), checkCanReview()]);
      } else {
      ...
      <div className="flex flex-col sm:flex-row gap-12">
        ...
        <div className="flex-1">
          ...
          <button
            ...
            className={`bg-black text-white px-8 py-3 text-sm ${productData.stock === 0 ? 'opacity-50 cursor-not-allowed' : 'active:bg-gray-700'}`}
      ...
      <div className="mt-20">
        ...
        {activeTab === 'reviews' && (
          <div className="flex flex-col gap-4 border px-6 py-6 text-sm text-gray-500">
            ...
            {cookies.userId ? (
              hasReviewed ? (
                <p className="text-red-500">You have already reviewed this product.</p>
              ) : canReview ? (
                <form onSubmit={handleReviewSubmit} className="space-y-4">
                  <div>
                    ...
                    <div className="flex gap-1">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <img
                          key={star}
                          src={reviewForm.rating >= star ? assets.star_icon : assets.star_dull_icon}
                          alt={`Star ${star}`}
                  ...
                  <button
                    ...
                    className={`w-full bg-black text-white p-2 rounded ${reviewLoading ? 'opacity-50 cursor-not-allowed' : 'hover:bg-orange-500'} transition-colors`}
                  >
                    {reviewLoading ? 'Submitting...' : 'Submit Review'}
                  </button>
                </form>
              ) : (
                <p className="text-red-500">You can only review products you have purchased and received.</p>
              )
            ) : (
              <p className="text-red-500">Please log in to submit a review.</p>
            )}
            <h3 className="text-lg font-medium text-black mt-6">Current Reviews</h3>
            {reviewLoading ? (
              <p>Loading reviews...</p>
            ) : reviews.length > 0 ? (
