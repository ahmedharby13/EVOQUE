...
const PlaceOrder: React.FC = () => {
      ...
      if (sessionId && orderId && location.search.includes('payment_intent')) {
        try {
          ...
          if (response.data.success) {
            ...
            navigate('/orders');
            // Fetch updated cart data to ensure sync with server
            await getCartData();
          } else {
  ...
  }, [location, token, csrfToken, fetchCsrfToken, backendUrl, navigate, setCartItem, getCartData, isAuthenticated]);
      ...
      switch (payMethod) {
        ...
        case 'stripe': {
          ...
          const response = await axios.post(`${backendUrl}/api/order/stripe`, orderData, {
            headers: { Authorization: `Bearer ${token}`, 'X-CSRF-Token': currentCsrfToken, Origin: window.location.origin },
            withCredentials: true,
          });
          if (response.data.success) {
            setCartItem({});
